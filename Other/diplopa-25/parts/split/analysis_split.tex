...
Найдём значения точки равновесия. Используем обозначения \eqref{flow_sub} и \(\alpha'_0 = \alpha^b_s\). Найдём \(N_0\):
...
Если \(q+1 \equiv s \mod 2\) тогда можно обойти третье уравнение в \eqref{double_stab_system}.
\begin{itemize}
    \item При \(q = 2p\): \( N_{q+1} = H_{2p} (N_1 - f_{2p}) \) откуда из \(N_{q+1} = 0\) следует \(N_1 = f_{2p} > 0\), значит из \eqref{split_lin_N0_0} выполняется всегда.
    
    \item При \(q=2p-1\): \(N_{q+1} =H_{2p-1} (N_0 - f_{2p-1}) = 0\) откуда \(N_0 = f_{2p-1}\). Тогда
...
Если \(r = 2l - 1\), тогда \(N'_{r+1} = N'_{2l} = H'_{2l-1} (N_s - f'_{2l-1}) = 0\), \(N_{s} = f'_{2l-1}\).
\begin{itemize}
    \item При \(s = 2u\): \(N_{2u} = H_{2u-1} (N_0 - f_{2u-1})\), \(N_0 = \frac{f'_{2l-1}}{H_{2u-1}} + f_{2u-1}\). Условие устойчивости выполняется аналогично \eqref{split_f2pm1_stab} всегда.
    
    \item При \(s = 2u-1\): \(N_{2u-1} = H_{2u-2} (N_1 - f_{2u-2})\), \( N_1 = \frac{f'_{2l-1}}{H_{2u-2}} + f_{2u-2} > 0\), значит из \eqref{split_lin_N0_0} выполняется всегда.
\end{itemize}